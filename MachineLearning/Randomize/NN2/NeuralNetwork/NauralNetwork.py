...
class NN:
    def __init__(self, file, alpha, num_hidden, maxLines, multipleText, removeLowVariance, hiddenLayerSizes=None, random_state=None, max_iter=500, saveIndex = 0):
        # Find all the words
        ...
        if saveIndex > 0:
            with open(f"D:/School/GT/MachineLearning/Randomize/NN2/NeuralNetwork/Sarcasm_train_group_{saveIndex}.csv", 'w') as f:
                for i in range(len(valueList)):
                    for j in range(len(self.dct.keys())):
                        if sampleList[i][j] == -1:
                            sampleList[i][j] = 0
                        f.write("" + str(sampleList[i][j]) + ",")
                    if valueList[i] == -1:
                        valueList[i] = 0
                    f.write("" + str(valueList[i]) + "\n")
            with open(f"D:/School/GT/MachineLearning/Randomize/NN2/NeuralNetwork/Sarcasm_test_list_{saveIndex}.csv", 'w') as f:
                with open(f"D:/School/GT/MachineLearning/SupervisedLearning/Sarcasm/Sarcasm_train_set_{saveIndex}.csv", 'r', newline='') as r:
                    reader = csv.reader(r, delimiter=',', quotechar='"')
                    for row in reader:
                        value = int(row[1])
                        if value == 1:
                            continue
                        ss = [-1 for i in range(len(self.dct.keys()))]
                        words = row[0].split(' ')
                        for word in words:
                            if word in self.dct:
                                ss[self.dct[word]] = 1
                        ...
                        if value == 2:
                            value = 1
                        f.write("" + str(value) + "\n")
            with open(f"D:/School/GT/MachineLearning/Randomize/NN2/NeuralNetwork/Sarcasm_final_set_{saveIndex}.csv", 'w') as f:
                with open(f"D:/School/GT/MachineLearning/SupervisedLearning/Sarcasm/Sarcasm_test_set.csv", 'r', newline='') as r:
                    reader = csv.reader(r, delimiter=',', quotechar='"')
                    for row in reader:
                        value = int(row[1])
                        if value == 1:
...
if True:
    ...
    for i in range(1,5):
        ...
        for j in range(1,5):
            if i == j: continue
            fileList.append(f"D:/School/GT/MachineLearning/SupervisedLearning/Sarcasm/Sarcasm_train_set_{j}.csv")
        d = NN(fileList, alpha, num_hidden, num, False, True, (5,), random_state, 80, i)
